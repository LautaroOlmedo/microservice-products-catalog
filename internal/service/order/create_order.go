...
/*func (s *Service) CreateOrder(ctx context.Context, productID string, quantity int) error {
	return s.TransactionManager.WithTransaction(ctx, func(txCtx context.Context) error {
		...
		if err := s.ProductService.UpdateProductStock(txCtx, productID, newStock); err != nil {
			return err
		}
		...
		return s.StorageRepository.CreateOrder(txCtx, *order)
	})
}*/
